...
//For Degree List
router.post('/DegreeList', (req, res) => {
            ...
            },  
            {
            ...
            },      
             {
                $match: { 
                    "ld.isActive": true,
                    "ld.isDeleted": false, 
                    "ld.lookupCode": constant.LookupCode.Degree,  
               }
            }, 
            {
...
//For Add Edit in Degree 
router.post('/AddUpdateDegree', (req, res) => {
    ...
    else
    {
        if(req.body.lookupDetailsId.trim() == "")
        {
            ...
            var lookupDetails = new LookupDetails({  
                lookupCode : constant.LookupCode.Degree,  
                isActive : true,
                isDeleted : false, 
        ...
        else
        {
            ...
            LookupDetails.count({ _id : ObjectId(req.body.lookupDetailsId), isActive : true, isDeleted : false }, async (err, IsExistDegree) => {
                console.log(IsExistDegree);
                    ...
                    if(IsExistDegree > 0)
                    {
                        ...
                        await req.body.lookupLangDetail.forEach( async function(item){
                            ...
                            LookupLangDetails.update(
                                ...
                                { $set: { lookupValue : item.degreeName, updatedBy: ObjectId(req.headers.userId), updatedDateTime : new Date() } }, function(err, lookupLangDetailsDoc) {
...
//For View Degree Details
router.post('/ViewDegree', (req, res) => {
...
// For Delete Degree Details 
router.post('/DeleteDegree', (req, res) => {
        ...
        {  
            if(!err)
            {
                if(lookupDetails.length)
                {
                    ...
                    //check this degree use or not if use then you can not delete it other wise delete degree
                    ProviderDetails.find({ degree : ObjectId(req.body.lookupDetailsId), isActive : true, isDeleted : false }, function(err, result) {
                        if (!err)  
                        {
                            
                            console.log("degree : " + req.body.lookupDetailsId)
                            console.log("Length : " + result.length)
                            console.log("Result : " + result)
                            
                            if(result.length > 0)
                            {
                                res.send(ResponseDTO.ErrorMassage(constant.ErrorCode.NotDeleteUseDegree, constant.ErrorMsg.NotDeleteUseDegree, {} ));     
                            }
                            else
                            {
